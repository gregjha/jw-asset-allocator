...
const split = computed(() =>
  store.splitFor(usdAmount.value, primaryCoin.value, secondaryCoin.value),
)
...
<template>
  <div class="split-card">
    ...
    <div v-if="isFetching && !split" class="status-text" aria-live="polite">
      Fetching live conversions...
    </div>
    ...
    <div v-else-if="split" class="split-results" aria-live="polite" aria-atomic="true">
      ...
      <div class="split-row">
        ...
        <span class="split-row__value"
          >{{ split.secondaryAmount.toFixed(8) }} {{ secondaryCoin }}</span
        >
      </div>
    </div>
...
.split-card {
  ...
  box-shadow:
    0 6px 10px -2px rgb(var(--color-shadow-rgb) / 0.25),
    0 24px 48px rgb(var(--color-shadow-rgb) / 0.3);
